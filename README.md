<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=220&color=0:0f172a,50:1e293b,100:334155&text=Bruno%20Rafael&fontColor=ffffff&fontSize=42&fontAlignY=38&desc=Desenvolvedor%20Fullstack%20em%20forma%C3%A7%C3%A3o&descAlignY=58&animation=fadeIn" width="100%" />
</p>

<h1 align="center">Bruno Rafael</h1>

<p align="center">
  Desenvolvedor Fullstack em formação • HTML • CSS • JavaScript • React
</p>

<p align="center">
  <a href="https://github.com/SEU-USUARIO">
    <img src="https://img.shields.io/badge/GitHub-121013?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <a href="https://www.linkedin.com/in/SEU-LINKEDIN">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Dispon%C3%ADvel%20para-Freelas-16a34a?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Foco-Fullstack-2563eb?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Stack-React-61DAFB?style=for-the-badge&logo=react&logoColor=000000" />
</p>

---

## Sobre mim

<table>
  <tr>
    <td width="230" align="center">
      <img src="./assets/profile.jpg" width="200" alt="Foto de perfil Bruno Rafael" />
    </td>
    <td>
      <p>
        Sou <strong>Bruno Rafael</strong>, estudante de <strong>Análise e Desenvolvimento de Sistemas</strong>,
        com formação técnica em informática e foco na construção de projetos reais para portfólio.
      </p>
      <p>
        Estou evoluindo como <strong>Desenvolvedor Fullstack em formação</strong>,
        com experiência em <strong>HTML, CSS, JavaScript e React</strong>,
        buscando criar aplicações funcionais, organizadas e com boa experiência de uso.
      </p>
      <p>
        Meu objetivo é ganhar experiência prática no mercado, desenvolver projetos para clientes
        e avançar de forma consistente na área de tecnologia.
      </p>
    </td>
  </tr>
</table>

---

## Stack principal

<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,react,git,github,vscode" />
</p>

---

## Objetivo atual

- Construir portfólio com projetos reais
- Desenvolver interfaces modernas e responsivas
- Criar aplicações com React
- Fazer trabalhos freelancer para acelerar experiência prática
- Evoluir tecnicamente como desenvolvedor Fullstack

---

## Contato

<p align="center">
  <a href="https://github.com/brndenh">
    <img src="https://skillicons.dev/icons?i=github" height="45" alt="GitHub" />
  </a>
  &nbsp;&nbsp;
  <a href="https://www.linkedin.com/in/brndenh">
    <img src="https://skillicons.dev/icons?i=linkedin" height="45" alt="LinkedIn" />
  </a>
</p>

<p align="center">
  <a href="https://github.com/brndenh"><strong>GitHub</strong></a> 
  <a href="https://www.linkedin.com/in/brndenh"><strong>LinkedIn</strong></a> 
</p>

---

<details>
  <summary><strong>Mais informações</strong></summary>
  <br>

- Estudante de ADS
- Técnico em Informática
- Conhecimento em HTML, CSS, JavaScript e React
- Construindo experiência com projetos reais e freelas
- Foco em crescimento consistente como desenvolvedor Fullstack

</details>
